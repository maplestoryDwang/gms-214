# A server emulator for Maplestory GMS v214.

原仓库地址：https://github.com/CoREse/gms214server


## 服务启动步骤：
1. 获取v214-server源码（本仓库）
2. JDK 17 
3. 导入sql数据，已修改成mysql8数据库，请提前安装mysql8数据库 
4. 修改配置，启动服务端，不报错即可

## 客户端启动：
1. 获取V214-client
2. AuthHook_v214 编译获取ijl15.dll，放在214client目录下覆盖 
3. launcher 编译获取v214.exe  登陆器用于注册和登录服务
4. v214.exe 放到客户端目录下
5. 点击v214.exe启动登陆器，点击注册账号，然后点击登录。注意查看服务端有没有收到发送的包。确认连接是否正常


hook和登陆器获取：https://github.com/Toxocious/Moonlight


## 注意事项
1. 为了更新日志版本。使用了lombok，IDEA安装lombok插件、设置搜索打开 Enable annotation processing
2. IDEA设置项目的resources文件夹。否则读不到配置文件路径
3. v214.exe 每次重启完服务器都要重新启动，自己编译的不需要。自己编译的版本可以直接创建socket
4. resources/clientFile 放了编译好的hook和launcher 以及字符串简体中文的汉化



## 管理员指令
1. 刷道具：!item item_id item_num
999个核心宝石
eg:!item 2435719 999
2. 开核心：@nodes
3. 技能5sCD
4. !map map_id


---
## 早期更新记录
1. 更新日志打印，添加查看包处理日志、unknow日志优化打印
2. 更新汉化
3. 更新V矩阵

---
## 更新记录
### 2025-11-10
1. 修正商场的包头，解决商城商品无法拿出的问题
2. 修复Zakum现在可以挑战！
![](asset/zakum.png)
3. 修复遇到的脚本问题
4. quick move 新增V技能大师NPC

### 2025-11-14
1. 修复黑龙BOSS可以正常挑战、显示残骸
   ![](asset/hontail.png)
2. 修复套装和装备血量导致人物血量异常无法加满的问题


### 2025-11-21
1.修复跑商，基本已经修复完成。可以正常
所有NPC均已经添加，包括盗贼，实现完了非常僵硬，感觉可以在进出传送门上加回调。而不是进入地图之后执行脚本。


入口：万能NPC -》 活动地图进入

![](asset/tradeking.png)
![](asset/tradeking3.png)
![](asset/tradeking2.png)

### 2025-11-30
1. 修复记忆魔方：黑魔方、白魔方
2. 六角魔方修复（六角魔方为任务）
![](asset/cube.png)

### 2025-12-9
1. 修复鲁塔比斯BOSS
2. 修复麦格纳斯BOSS正常挑战
![](asset/magnus.png)




## 下一步计划
1. 解决分辨率的问题（使用插件已解决）
2. 重构项目
```angular2html
项目架构：按照handlers进行拆分
1. 数据库项目
2. 处理包头Handler子项目
一些不清楚的处理逻辑也先丢这里
handle文件夹处理包
Packet 文件夹返回包
3. 加载处理wz数据等静态数据子项目
dat数据，wz数据、常量数据等
4. 用户相关 client
5.  处理技能
6.  脚本实现项目，包含物品使用，地图、任务
7.  world文件夹功能：拍卖、商城、
8.  ETC项目，实现ETC.wz里面活动等其他功能
9. 基础服务

其他重构
修改继承调用地狱。改成实现
修改技能也能脚本执行
重构BOSS
```
3. 关于装备。本身应该是做任务就能获取装备，然后打BOSS是爆一些强化装备的内容。这种模式比较好。而不是装备要靠打BOSS才能获得，这样打不过的永远都打不过。官服就是这样。

