...
import net.swordie.ms.traceking.*;
...
public class EtcData {
    ...
    private static Map<Integer, MapObjectInfo> mapObjectInfoMap = new HashMap<>();
     ...
     */
    public static void loadMapObjectInfoFromWz() {
//        String wzDir = "E:\\javaguide\\214\\wz\\xml\\wz\\Etc.wz/NihalTrade.img.xml";
        String wzDir = ServerConstants.WZ_DIR + "/Etc.wz/MapObjectInfo.img.xml";
        File dir = new File(wzDir);
        Node root = XMLApi.getFirstChildByNameBF(XMLApi.getRoot(dir), "MapObjectInfo.img");
        List<Node> nodes = XMLApi.getAllChildren(root);
        for (Node mainNode : nodes) {
            int mapID = Integer.valueOf(XMLApi.getNamedAttribute(mainNode, "name"));
            Node npc = XMLApi.getFirstChildByNameBF(mainNode, "npc");
            Node mob = XMLApi.getFirstChildByNameBF(mainNode, "mob");
            List<Integer> npcs = new ArrayList<>();
            List<Integer> mobs = new ArrayList<>();

            if (npc != null) {
                List<Node> npcLists = XMLApi.getAllChildren(npc);
                for (Node npcList : npcLists) {
                    int npcIndex = Integer.valueOf(XMLApi.getNamedAttribute(npcList, "name"));
                    int npcValue = Integer.valueOf(XMLApi.getNamedAttribute(npcList, "value"));
                    npcs.add(npcValue);
                }
            }

            if (mob != null) {
                List<Node> mobLists = XMLApi.getAllChildren(mob);
                for (Node mobList : mobLists) {
                    int index = Integer.valueOf(XMLApi.getNamedAttribute(mobList, "name"));
                    int value = Integer.valueOf(XMLApi.getNamedAttribute(mobList, "value"));
                    mobs.add(value);
                }
            }
            MapObjectInfo mapObjectInfo = new MapObjectInfo(npcs, mobs);
            mapObjectInfoMap.put(mapID, mapObjectInfo);


//            List<Node> mobists = XMLApi.getAllChildren(mob);
...
//        String wzDir = "E:\\javaguide\\214\\wz\\xml\\wz\\Etc.wz/NihalTrade.img.xml";
        ...
        Node workList = XMLApi.getFirstChildByNameBF(worker, String.valueOf(TraceKingQuestRxCode.WORKER.getVal()));
        List<Node> workNodes = XMLApi.getAllChildren(workList);
        ArrayList<TraceKingWorker> workers = new ArrayList<>();
        for (Node workNode : workNodes) {
            ...
            TraceKingWorker traceKingWorker = new TraceKingWorker(index, npcID, speedAdd, jumpAdd, weightAdd);
            workers.add(traceKingWorker);
        }
        traceKillMap.put(TraceKingQuestRxCode.WORKER.getVal(), workers);


        Node riding = XMLApi.getFirstChildByNameBF(node, "riding");
        ArrayList<TraceKingRiding> ridingList = new ArrayList<>();
        ...
        ridingList.add(new TraceKingRiding(0, riding0Value));
        ...
        ridingList.add(new TraceKingRiding(1, riding1Value));
        ...
        ridingList.add(new TraceKingRiding(2, riding2Value));
        traceKillMap.put(TraceKingQuestRxCode.RIDING.getVal(), ridingList);


        Node item = XMLApi.getFirstChildByNameBF(node, "item");
        List<Node> itemList = XMLApi.getAllChildren(item);
        ArrayList<TraceKingItem> items = new ArrayList<>();
...
//            }
            ...
            TraceKingItem traceKingItem = new TraceKingItem(itemId, qr, qrEx);
            items.add(traceKingItem);
        }

        List<TraceKingItem> list15323 = items.stream().filter(traceKingItem -> traceKingItem.getQr() == TraceKingQuestRxCode.ITEM_15323.getVal()).toList();
        List<TraceKingItem> list15322 = items.stream().filter(traceKingItem -> traceKingItem.getQr() == TraceKingQuestRxCode.ITEM_15322.getVal()).toList();
        List<TraceKingItem> list15347 = items.stream().filter(traceKingItem -> traceKingItem.getQr() == TraceKingQuestRxCode.ITEM_15347.getVal()).toList();
        List<TraceKingItem> list15346 = items.stream().filter(traceKingItem -> traceKingItem.getQr() == TraceKingQuestRxCode.ITEM_15346.getVal()).toList();
        List<TraceKingItem> list15345 = items.stream().filter(traceKingItem -> traceKingItem.getQr() == TraceKingQuestRxCode.ITEM_15345.getVal()).toList();
        List<TraceKingItem> list15344 = items.stream().filter(traceKingItem -> traceKingItem.getQr() == TraceKingQuestRxCode.ITEM_15344.getVal()).toList();


        traceKillMap.put(TraceKingQuestRxCode.ITEM_15323.getVal(), list15323);
        traceKillMap.put(TraceKingQuestRxCode.ITEM_15322.getVal(), list15322);
        traceKillMap.put(TraceKingQuestRxCode.ITEM_15347.getVal(), list15347);
        traceKillMap.put(TraceKingQuestRxCode.ITEM_15346.getVal(), list15346);
        traceKillMap.put(TraceKingQuestRxCode.ITEM_15345.getVal(), list15345);
        traceKillMap.put(TraceKingQuestRxCode.ITEM_15344.getVal(), list15344);
        ...
        List<TraceKingGold> goldList = new ArrayList<>();
        goldList.add(new TraceKingGold(-1, qr, qrEx));
        traceKillMap.put(TraceKingQuestRxCode.GOLD.getVal(), goldList);
    ...
    public static MapObjectInfo getMapObjectById(int mapId) {
        if (mapObjectInfoMap.isEmpty()) {
            loadMapObjectInfoFromWz();
        }
        if (mapObjectInfoMap.containsKey(mapId)) {
            return mapObjectInfoMap.get(mapId);
        }
        return null;
    }
